...
class Parser:
    ...
    def isNumber(self):
        ...
        str1 = ''
        while self.pos < len(self.expression):
            code = str(self.expression[self.pos])
            if (code >= '0' and code <= '9') or code is '.':
                str1 += str(self.expression[self.pos])
                self.pos += 1
                self.tokennumber = float(str1)
                r = True
            else:
                break
        return r
    ...
    def isString(self):
        ...
        str1 = ''
        startpos = self.pos
        if self.pos < len(str(self.expression)) and str(self.expression[self.pos]) == '':
            self.pos += 1
            while self.pos < len(str(self.expression)):
                code = str(self.expression[self.pos])
                if code != '\'' or str1[-1] == '\\':
                    str1 += str(self.expression[self.pos])
                    self.pos += 1
                    ...
                    self.pos += 1
                    self.tokennumber = self.unescape(str1, startpos)
                    r = True
                    break
        return r
    ...
    def isConst(self):
        for i in self.consts:
            L = len(i)
            str1 = str(self.expression[self.pos:self.pos+L])
            if i == str1:
    ...
    def isOperator(self):
        ...
        if code is '+':
            ...
            if str(self.expression[self.pos]) is '|':
                self.pos += 1
                self.tokenprio = 0
                self.tokenindex = '||'
            else:
    ...
    def isSign(self):
        code = str(self.expression[self.pos - 1])
    ...
    def isOp1(self):
        str1 = ''
        for i in range(self.pos, len(str(self.expression))):
            c = str(self.expression[i])
            if c.upper() == c.lower():
                if i == self.pos or (c != '_' and (c < '0' or c > '9')):
                    break
            str1 += c
        if len(str1) > 0 and str1 in self.ops1:
            self.tokenindex = str1
            self.tokenprio = 5
            self.pos += len(str1)
            return True
        return False
        ...
        for i in range(self.pos, len(str(self.expression))):
            ...
            if c.upper() == c.lower():
                if i == self.pos or (c != '_' and (c < '0' or c > '9')):
                    break
            str1 += c
        if len(str1) > 0 and (str1 in self.ops2):
            self.tokenindex = str1
            self.tokenprio = 5
            self.pos += len(str1)
            return True
    ...
    def isComment(self):
        code = str(self.expression[self.pos - 1])
        if code is '/' and str(self.expression[self.pos]) is '*':
