...
class GenUI(wx.Frame):
    ...
    def init_ui(self):
        # функция
        self.row = 0
        grid_function = wx.GridBagSizer(hgap=5, vgap=5)
        ...
        self.add_row_to_grid_bag(grid_function, function_lbl)
        self.add_row_to_grid_bag(grid_function, self.function)
        self.add_row_to_grid_bag(grid_function, self.extremum_rbox)

        self.extremum_rbox.Bind(wx.EVT_KEY_DOWN, self.OnKeyDown)
        self.function.Bind(wx.EVT_KEY_DOWN, self.OnKeyDown)

        # поле поиска
        grid_search_field = wx.GridBagSizer(hgap=5, vgap=5)
        box_search_field = wx.StaticBoxSizer(wx.StaticBox(self, -1, u'Поле поиска'))
        box_search_field.Add(grid_search_field)
        self.add_row_to_grid_bag(grid_function, box_search_field)
        ...
        self.row = 0
        self.add_row_to_grid_bag(grid_search_field, wx.StaticText(self), from_lbl, to_lbl)
        self.add_row_to_grid_bag(grid_search_field, x_lbl, self.minx, self.maxx)
        self.add_row_to_grid_bag(grid_search_field, y_lbl, self.miny, self.maxy)
        ...
        # настройки
        self.row = 0
        grid_genetic = wx.GridBagSizer(hgap=5, vgap=5)

        # кроссовер
        cross_p_lbl = wx.StaticText(self, label=u"Вероятность кроссовера:")
        self.cross_p = wx.TextCtrl(self, size=(50, -1))
        self.cross_p_slider = wx.Slider(self, minValue=0, maxValue=10 ** GenUI.PRECISION, size=(100, -1),
                                        style=wx.SL_HORIZONTAL)

        self.add_row_to_grid_bag(grid_genetic, cross_p_lbl, self.cross_p, self.cross_p_slider)

        self.better_bind(wx.EVT_SCROLL, self.cross_p_slider, self.OnScroll, text_ctrl=self.cross_p, is_float=True)
        self.better_bind(wx.EVT_TEXT, self.cross_p, self.OnTextChanged, text_ctrl=self.cross_p,
                         slider=self.cross_p_slider, is_float=True)
        ...
        self.mut_p_slider = wx.Slider(self, minValue=0, maxValue=10 ** GenUI.PRECISION / 10, size=(100, -1),
                                      style=wx.SL_HORIZONTAL)

        self.add_row_to_grid_bag(grid_genetic, mut_p_lbl, self.mut_p, self.mut_p_slider)

        self.better_bind(wx.EVT_SCROLL, self.mut_p_slider, self.OnScroll, text_ctrl=self.mut_p, is_float=True)
        self.better_bind(wx.EVT_TEXT, self.mut_p, self.OnTextChanged, text_ctrl=self.mut_p, slider=self.mut_p_slider,
                         is_float=True)
        ...
        self.add_row_to_grid_bag(grid_genetic, size_lbl, self.size, self.size_slider)
        ...
        self.add_row_to_grid_bag(grid_genetic, elite_lbl, self.elite, self.elite_slider)
        ...
        self.blx_slider = wx.Slider(self, minValue=0, maxValue=10 ** GenUI.PRECISION, size=(100, -1),
                                    style=wx.SL_HORIZONTAL)

        self.add_row_to_grid_bag(grid_genetic, blx_lbl, self.blx, self.blx_slider)
        self.better_bind(wx.EVT_SCROLL, self.blx_slider, self.OnScroll, text_ctrl=self.blx, is_float=True)
        self.better_bind(wx.EVT_TEXT, self.blx, self.OnTextChanged, text_ctrl=self.blx, slider=self.blx_slider,
                         is_float=True)
        ...
        self.add_row_to_grid_bag(grid_genetic, maxgen_lbl, self.maxgen, self.maxgen_slider)
        ...
        self.Bind(wx.EVT_BUTTON, lambda _: self.log.Clear(), self.clear_btn)
        ...
        genetic.AddMany((grid_genetic, sampling))

        options = wx.FlexGridSizer(cols=2,rows=0,vgap=0,hgap=10)
        options.AddMany((grid_function, genetic))
    ...
    def add_row_to_grid_bag(self, grid, *controls):
        for i, control in enumerate(controls):
            grid.Add(control, pos=(self.row, i))
        self.row += 1

    def better_bind(self, event_type, instance, handler, *args, **kwargs):
        self.Bind(event_type, lambda event: handler(event, *args, **kwargs), instance)
    ...
    def OnGoClick(self, e):
        if self.function.Value:
            ...
            history = g.start(show_plot=False, print_stats=self.stats_chk.IsChecked(),
